...
category_replacements = {
    "HOME AND KITCHEN I": "GROCERY I","HOME AND KITCHEN II": "GROCERY II","PLAYERS AND ELECTRONICS":"PRODUCE","LADIESWEAR":"LINGERIE","BOOKS": "SEAFOOD","SCHOOL AND OFFICE SUPPLIES": "POULTRY",
}
...
async def predict_sales(input_data: PredictionInput):
    ...
    # ✅ Make Predictions
    future_dates = pd.date_range(start="2025-03-26", end="2025-08-31", freq="2W")  # 2-week intervals
    predictions = model.predict(x_test_reshaped[:len(future_dates)]).flatten().tolist()
    if category in category_replacements:
            new_category = category_replacements[category]
            logging.info(f"🔄 Replacing '{category}' with '{new_category}' for prediction.")
            return await predict_sales(PredictionInput(category=new_category))
    return {
        "category": category,
